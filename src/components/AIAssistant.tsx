...
onClick={closeAssistant}
            ...
            <div ref={scrollRef} className="flex-1 p-4 overflow-y-auto space-y-4">
              {messages.map((m, i) => (
                <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
                  <div className={`max-w-[85%] p-3 sm:p-4 rounded-md text-xs sm:text-sm font-medium leading-relaxed ${m.role === 'user'
                    ? 'bg-ink-primary text-paper rounded-br-none'
                    : 'bg-elevated border border-rule/12 text-ink-primary rounded-bl-none'
                    }`}>
                    {m.isLive && (
              ...
              {isThinking && (
                <div className="flex items-start">
                  <div className="bg-elevated border border-rule/12 text-ink-primary rounded-md rounded-bl-none p-4 flex items-center gap-2">
                    <div className="w-1.5 h-1.5 bg-ink-secondary/50 rounded-full animate-bounce"></div>
                    <div className="w-1.5 h-1.5 bg-ink-secondary/50 rounded-full animate-bounce [animation-delay:0.2s]"></div>
                    <div className="w-1.5 h-1.5 bg-ink-secondary/50 rounded-full animate-bounce [animation-delay:0.4s]"></div>
                  </div>
                </div>
              )}
            </div>
            ...
            {/* Input Area */}
            <div className="p-3 bg-elevated/50 border-t border-rule/12">
              <div className="relative flex items-center">
                <input
                  ...
                  className="w-full bg-paper border border-rule/12 rounded-full pl-4 pr-12 py-3 text-xs sm:text-sm focus:outline-none focus:border-accent transition-colors text-ink-primary placeholder:text-ink-secondary"
                />
                <button
                  onClick={handleSend}
                  disabled={!input.trim() || isThinking}
                  className="absolute right-1.5 p-2 bg-ink-primary text-paper rounded-full hover:bg-accent transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
