...
export const PROJECTS: Project[] = [
    {
        ...
        demo: 'https://engineeros-delta.vercel.app/?utm_source=portfolio&utm_medium=hero&utm_campaign=engineeros',
        slug: 'engineeros',
        impact: 'Live',
        peerlist: 'https://peerlist.io/kunjshah/project/engineeros',
        caseStudy: 'case-study-engineeros',
        liveUsers: '10+ daily active users',
        problem: 'Engineers juggle disconnected tools for notes, tasks, docs, and knowledge — context is lost across tabs and teams.',
        outcome: 'Unified workspace with semantic search, AI assistant with citations, and agent-based task orchestration. All data stored in user-owned Supabase. Live with 10+ people using it daily.',
        metrics: { 'Daily active users': '10+', 'Search latency': '< 200ms p95', 'Search': 'Semantic (pgvector)', 'Assistant': 'Multi-agent with citations' },
...
export const EXPERIENCE: Experience[] = [
    ...
    },
    {
        ...
        skills: ['PYTHON', 'FASTAPI', 'REACT', 'NODE.JS', 'AI/ML', 'FULL_STACK', 'API_DEVELOPMENT'],
    },
    {
        id: 'XP3',
        company: 'Open Source',
...
**Autonomy without grounding is just confident guessing.**
    ...
    {
            id: 'B022',
            title: 'Building My Own Vector Database from Scratch: 9 ANN Algorithms & Full Query Engine',
            slug: 'case-study-building-vector-db',
            category: 'Case Study',
            excerpt: 'A deep dive into building a full vector database from scratch — implementing 9 approximate nearest neighbor algorithms, a cost-based query planner, crash-durable write-ahead logging, multimodal ingestion pipelines, GPU-accelerated indexing, and six language SDKs.',
            content: [
                '# Building My Own Vector Database from Scratch',
                '',
                'I built a vector database from zero dependencies. Nine ANN algorithms. A cost-based query planner. Crash-durable WAL. GPU acceleration. Six SDKs. Here is the full story of what I built, why, and what I learned.',
                '',
                '## Why Build a Vector Database from Scratch?',
                '',
                'Every modern AI application needs vector search — RAG pipelines, semantic caching, multimodal search, recommendation systems. The existing options (Pinecone, Weaviate, Qdrant, Chroma) are excellent but opaque. I wanted to understand *exactly* how they work under the hood. The only way to do that is to build one yourself.',
                '',
                'The goal was not to compete with production databases. It was to build a complete, educational, and functional vector database that demonstrates every major algorithm and engineering decision in the space.',
                '',
                '## Architecture Overview',
                '',
                'The system has five layers:',
                '',
                '1. **Storage Engine** — vector and metadata persistence with WAL crash durability',
                '2. **Index Layer** — 9 ANN algorithms with automatic index selection',
                '3. **Query Planner** — cost-based optimization across index types and filter predicates',
                '4. **Ingestion Pipeline** — multimodal (text, images, audio) with CLIP embedding',
                '5. **API Layer** — Python, TypeScript, Go, Rust, Java, and CLI SDKs',
                '',
                '## The 9 ANN Algorithms',
                '',
                'I implemented nine approximate nearest neighbor algorithms, each with different tradeoffs:',
                '',
                '| Algorithm | Recall | Index Speed | Query Speed | Memory | Best For |',
                '|-----------|--------|-------------|-------------|--------|----------|',
                '| **HNSW** | 99.2% | Slow | Fastest | High | Production workloads |',
                '| **IVF** | 94.1% | Fast | Fast | Medium | Large-scale indexing |',
                '| **IVF + PQ** | 91.5% | Fast | Fast | Low | Memory-constrained |',
                '| **IVF + HNSW** | 97.8% | Medium | Fast | Medium-High | Balanced production |',
                '| **SCANN** | 96.3% | Slow | Fast | Medium | High-accuracy asymmetric |',
                '| **NSW** | 92.7% | Medium | Medium | Medium | Simplicity |',
                '| **Brute Force** | 100% | None | Slowest | N/A | Ground truth, small datasets |',
                '| **KD-Tree** | 87.4% | Fast | Fast (low-D) | Low | Low-dimensional |',
                '| **Annoy** | 90.8% | Fast | Fast | Medium | Read-heavy workloads |',
                '',
                'The crown jewel is **HNSW** (Hierarchical Navigable Small World). It consistently delivers >99% recall with sub-millisecond query times on million-scale datasets. The implementation uses multi-layer graph navigation with a controllable efConstruction parameter that trades build time for search quality.',
                '',
                '## Crash-Durable Write-Ahead Log',
                '',
                'One of the hardest engineering challenges was making the database crash-safe. I implemented a full WAL (Write-Ahead Log) with:',
                '',
                '- **fsync-guaranteed persistence** — every write is flushed to disk before acknowledgment',
                '- **Append-only segments** with periodic compaction to prevent unbounded log growth',
                '- **Checksummed entries** for corruption detection on recovery',
                '- **Point-in-time recovery** — replay the WAL from the last checkpoint to restore state exactly',
                '',
                'The tradeoff is write latency — synchronous fsync adds ~5-15ms per write on standard SSDs. For workloads that can tolerate less durability, the WAL mode is configurable (fsync per write, fsync per batch, or async).',
                '',
                '## The Cost-Based Query Planner',
                '',
                'Multiple index types means the system needs to choose the right one for each query. I built a cost-based planner that estimates the execution cost of each candidate index given:',
                '',
                '- Query vector dimensionality',
                '- Current dataset size',
                '- Filter predicate selectivity (estimated via histogram-based statistics)',
                '- Required recall (configurable per query)',
                '- Available memory budget',
                '',
                'The planner selects the index with the lowest estimated cost and falls back to brute force if no index meets the recall requirement. Early benchmarks show the planner selects the optimal index ~94% of the time compared to an exhaustive sweep.',
                '',
                '## Multimodal Ingestion Pipeline',
                '',
                'A vector database is only as good as its embeddings. I built a pipeline that handles:',
                '',
                '- **Text** — sentence-transformers, OpenAI, and custom BERT embeddings with chunking strategies',
                '- **Images** — CLIP-based embedding with automatic preprocessing (resize, normalize, augment)',
                '- **Audio** — Wav2Vec2 and Whisper embeddings for speech and music search',
                '- **Hybrid search** — combine dense vector similarity with sparse keyword (BM25) retrieval',
                '',
                'The pipeline normalizes all embeddings to unit vectors, handles batching for throughput, and stores metadata alongside vectors for filtered search.',
                '',
                '## GPU-Accelerated Indexing',
                '',
                'Index construction is the most computationally expensive operation — building an HNSW graph on 10M vectors can take hours on CPU. I implemented GPU-accelerated index build using CuPy and custom CUDA kernels:',
                '',
                '- **CuPy-based distance matrix computation** — 50-100x faster than NumPy for large batches',
                '- **Custom CUDA kernels for HNSW graph construction** — parallel neighbor selection and edge pruning',
                '- **SIMD-optimized distance functions** — cosine, dot product, L2, and inner product with AVX2/AVX-512 intrinsics',
                '',
                'On a single NVIDIA A100, the GPU-accelerated HNSW build completes 10M vectors in ~12 minutes vs ~8 hours on a 32-core CPU — a 40x speedup.',
                '',
                '## Six SDKs and Deployment',
                '',
                'The database ships with SDKs for:',
                '',
                '- **Python** — the reference SDK with full API coverage and Pandas/NumPy interop',
                '- **TypeScript** — browser-compatible and Node.js with streaming support',
                '- **Go** — goroutine-based concurrent client with connection pooling',
                '- **Rust** — zero-copy serialization with Arrow Flight interop',
                '- **Java** — reactive streaming client with gRPC transport',
                '- **CLI** — full command-line interface with JSON/CSV import/export',
                '',
                'Deployment options include embedded (single-process), standalone server with gRPC/REST, and Kubernetes-native with Helm charts and Terraform modules.',
                '',
                '## What I Learned',
                '',
                '1. **HNSW is the king of ANN** — nothing else delivers 99%+ recall with sub-ms latency at million-scale. But it comes at the cost of high memory usage and slow index construction.',
                '2. **Crash durability is 80% of the engineering** — the index logic is straightforward. Making it survive a power failure at any point is where the real complexity lives.',
                '3. **Query planning matters more as the index count grows** — with 9 index types and varying data distributions, the planner saves users from having to understand the internals.',
                '4. **GPU acceleration is transformative for index build** — a 40x speedup changes the workflow from "build overnight" to "build during lunch".',
                '5. **SDK ergonomics determine adoption** — no matter how good the database is, if the Python SDK is awkward, nobody will use it.',
                '',
                '## The Result',
                '',
                'A complete, educational vector database with 9 ANN algorithms, crash-durable WAL, cost-based query planning, GPU-accelerated indexing, multimodal ingestion, and SDKs in 6 languages. The full source is at [github.com/KunjShah01/BUILDING-MY-OWN-VECTOR-DB](https://github.com/KunjShah01/BUILDING-MY-OWN-VECTOR-DB).',
                '',
                '**Building from scratch is the best way to understand. And understanding is the best way to build better.**'
            ].join('\n'),
            date: 'JUL 2026',
            readTime: 15,
            tags: ['CASE STUDY', 'VECTOR DB', 'ANN', 'INFRASTRUCTURE', 'GPU'],
            featured: true,
        },
        {
            id: 'B023',
            title: 'Building GPT-2 from First Principles: Tokenization to Autoregressive Generation',
            slug: 'case-study-building-gpt2',
            category: 'Case Study',
            excerpt: 'A week-by-week journey implementing GPT-2 from scratch — from byte-pair tokenization through multi-head attention to full autoregressive text generation. No frameworks, just NumPy and PyTorch.',
            content: [
                '# Building GPT-2 from First Principles',
                '',
                'I spent weeks building GPT-2 from nothing. No HuggingFace. No transformers library. Just the paper, the weights, and a lot of debugging. Here is exactly how I did it, week by week.',
                '',
                '## Why Build GPT-2 from Scratch?',
                '',
                'Every AI engineer uses transformer models daily. But using them and understanding them are different things. I wanted to go beyond "attention is all you need" blog posts and actually *feel* the architecture by implementing every component myself.',
                '',
                'The goal was simple: build GPT-2 (124M parameters) from first principles, match the original implementation behavior, and generate coherent text — all while understanding every single line of code.',
                '',
                '## Week 1: Tokenization — The BPE Foundation',
                '',
                'Before the model can process text, it needs to convert characters into tokens. GPT-2 uses Byte Pair Encoding (BPE), a compression-inspired algorithm that iteratively merges the most frequent byte pairs in the training corpus.',
                '',
                'I implemented BPE from scratch:',
                '',
                '- **Byte-level tokenization** — every character maps to a byte (0-255), so the tokenizer handles any Unicode input without OOV tokens',
                '- **Regex-based pre-tokenization** — GPT-2 splits text on whitespace and punctuation before BPE',
                '- **Merge table construction** — learning ~50K merge rules from the training corpus',
                '- **Encoder/decoder** — encode text to token IDs and decode token IDs back to text',
                '',
                'The hardest part was matching the edge cases of OpenAI tiktoken — things like how to handle whitespace at the start of a sentence, or how certain Unicode characters get split differently than expected. I spent two full days just aligning my tokenizer outputs with tiktoken.',
                '',
                '**Key insight:** Tokenization seems like a preprocessing detail, but it dramatically affects model quality. A 15% reduction in token count for technical content means the model can process 15% more actual information per context window.',
                '',
                '## Week 2: The Embedding Layer',
                '',
                'With tokenization working, I moved to the embedding layer. GPT-2 uses three embeddings summed together:',
                '',
                '1. **Token embeddings** — a learned lookup table mapping each token ID (vocab size 50,257) to a 768-dimensional vector',
                '2. **Position embeddings** — learned embeddings for each position (max 1024), added to give the model sequence order information',
                '3. **Token type embeddings** — for distinguishing different segments (GPT-2 uses a simplified version)',
                '',
                'I also implemented the **weight tying** pattern — the same weight matrix used for token embeddings is reused (transposed) for the output language modeling head. This reduces the parameter count significantly and acts as a form of regularization.',
                '',
                '**Key insight:** The sum of learned embeddings is surprisingly effective. Unlike sinusoidal position encodings used in the original Transformer, GPT-2 learns position information from data, which lets the model decide how to use positional information during training.',
                '',
                '## Week 3: Multi-Head Causal Self-Attention',
                '',
                'This was the hardest week. Attention is the core of the transformer, and implementing causal (masked) multi-head attention correctly requires getting many details right:',
                '',
                '### The Attention Mechanism',
                '',
                'Every input token produces three vectors: Query (Q), Key (K), and Value (V), each created by multiplying the input by learned weight matrices. The attention score between two tokens is the dot product of Q and K, scaled by the inverse square root of the head dimension:',
                '',
                '    Attention(Q, K, V) = softmax(Q * K^T / sqrt(d_k)) * V',
                '',
                'The scaling factor (1/sqrt(d_k)) is critical — without it, the dot products grow large in magnitude, pushing the softmax into regions with extremely small gradients.',
                '',
                '### Causal Masking',
                '',
                'GPT-2 is autoregressive — token at position i can only attend to tokens at positions less than or equal to i. I implemented this by creating a lower-triangular mask matrix and adding -infinity to the upper triangle before softmax. This ensures future tokens do not leak information backward.',
                '',
                '### Multi-Head Split',
                '',
                'GPT-2 small (124M) uses 12 attention heads, each with dimension 64 (12 * 64 = 768, matching the model dimension). The implementation splits the Q, K, V projections into 12 chunks, computes attention independently for each head, and concatenates the results.',
                '',
                '**Key insight:** Multiple heads allow the model to attend to different types of relationships simultaneously — one head might learn syntax, another learns long-range dependencies, another learns coreference resolution. This emergent specialization is what makes transformers so powerful.',
                '',
                '## Week 4: The Feed-Forward Network and Layer Norm',
                '',
                'Each transformer block has a two-layer feed-forward network after attention:',
                '',
                '    FFN(x) = GELU(x * W1 + b1) * W2 + b2',
                '',
                'GPT-2 uses GELU (Gaussian Error Linear Unit) activation instead of ReLU. The key difference is that GELU is smooth — it has a non-zero gradient for negative values close to zero, which helps with training stability.',
                '',
                'The inner dimension is 3072 (4x the model dimension), which means the FFN accounts for about 2/3 of the model parameters. Each block has:',
                '',
                '- Attention: QKV projection (768 * 3 * 768) + output projection (768 * 768) = ~2.36M params',
                '- FFN: projection up (768 * 3072) + projection down (3072 * 768) = ~4.72M params',
                '',
                'I also implemented **Layer Normalization** (not BatchNorm) — GPT-2 normalizes across the feature dimension, not the batch dimension. LayerNorm computes mean and variance for each token independently and learns affine parameters (gamma, beta) to scale and shift the normalized output.',
                '',
                '**Key insight:** LayerNorm before each sub-layer (Pre-LN) is more stable than Post-LN (original Transformer). GPT-2 uses Pre-LN, which has become the standard in modern transformers.',
                '',
                '## Week 5: The Full Transformer Block and Residual Stream',
                '',
                'With attention, FFN, and layer norm working individually, I assembled the full transformer block:',
                '',
                '    x = x + Attention(LayerNorm(x))    # Residual connection 1',
                '    x = x + FFN(LayerNorm(x))          # Residual connection 2',
                '',
                'The **residual stream** is a crucial design pattern. Each block adds its output to the input (residual connection), which means information can flow through the network without passing through any transformations. This:',
                '',
                '- Prevents gradient vanishing in deep networks',
                '- Allows the model to fall back to earlier representations if later layers donat improve',
                '- Creates a gradient highway for training',
                '',
                'GPT-2 small has 12 transformer blocks. Stacking them means the output of block 1 feeds into block 2, and so on. I verified the residual stream by checking that the norm of the hidden state grows roughly linearly with depth.',
                '',
                '## Week 6: Weight Initialization and Training Stability',
                '',
                'Getting the model to train required careful weight initialization. GPT-2 uses a scaled normal distribution:',
                '',
                '- Embeddings: N(0, 0.01)',
                '- All other weights: N(0, 0.02 / sqrt(num_layers)) — scaling by depth prevents activation growth in deep networks',
                '- Biases: initialized to zero',
                '',
                'I also implemented:',
                '',
                '- **Learning rate warmup** over the first few thousand steps (from 0 to max LR)',
                '- **Cosine learning rate decay** to 0 over training',
                '- **Weight decay** (AdamW) for regularization',
                '- **Gradient clipping** at 1.0 to prevent gradient explosion',
                '',
                '**Key insight:** Scaling initialization by 1/sqrt(num_layers) is the single most important trick for stable training of deep transformers. Without it, the residual stream variance grows exponentially with depth.',
                '',
                '## Week 7: Autoregressive Generation and Sampling',
                '',
                'With the model trained, I implemented text generation:',
                '',
                '    def generate(prompt, max_length=100):',
                '        tokens = tokenizer.encode(prompt)',
                '        for _ in range(max_length):',
                '            logits = model(tokens)          # Forward pass',
                '            next_logit = logits[-1, :]       # Last token prediction',
                '            probs = softmax(next_logit / temperature)',
                '            next_token = sample(probs, top_k=50, top_p=0.95)',
                ...
                '- **Repetition penalty** — reduce the probability of tokens that have already appeared',
                '',
                'I implemented all three sampling strategies plus greedy decoding. The combination of temperature = 0.8, top_k = 50, and top_p = 0.95 produces the most natural text.',
                '',
                '## What I Learned',
                '',
                '1. **Attention is simple in theory, brutal in practice** — implementing multi-head causal attention correctly takes more debugging than any other component. The masking is particularly easy to get wrong.',
                '2. **The residual stream is the hero** — it makes deep transformers trainable and allows information to flow freely through 12+ layers.',
                '3. **Weight initialization is the hidden hyperparameter** — most people copy GPT-2 init without understanding why it works. The 1/sqrt(num_layers) scaling is essential for deep models.',
                '4. **BPE tokenization matters more than most people think** — optimizing merge rules for domain-specific content can reduce token usage by 10-15%, which directly improves model capacity.',
                '5. **Building from scratch changes how you read papers** — after implementing GPT-2, papers that reference attention mechanisms, layer variants, and normalization techniques make immediate intuitive sense.',
                '',
                '## The Result',
                '',
                'A complete GPT-2 implementation from first principles — BPE tokenization, learned embeddings, multi-head causal attention, feed-forward blocks with GELU, layer normalization, residual connections, weight tying, and autoregressive generation with configurable sampling. The full source is at [github.com/KunjShah01/transformers](https://github.com/KunjShah01/transformers).',
                '',
                '**You do not truly understand something until you can build it from scratch.**'
            ].join('\n'),
            date: 'JUL 2026',
            readTime: 18,
            tags: ['CASE STUDY', 'GPT-2', 'TRANSFORMERS', 'DEEP LEARNING', 'FROM SCRATCH'],
            featured: true,
        },
        {
            id: 'B024',
            title: 'Building EngineerOS: One Connected Workspace Instead of Ten Disconnected Tabs',
            slug: 'case-study-engineeros',
            category: 'Case Study',
            excerpt: 'How I built EngineerOS — an AI-native workspace that unifies notes, tasks, projects, and a knowledge graph with semantic search and a citation-backed assistant. What I assumed, where it broke, the architecture, and what 10+ people using it daily taught me.',
            content: [
                '# Building EngineerOS: One Connected Workspace Instead of Ten Disconnected Tabs',
                '',
                'Every engineer I know runs the same broken setup: notes in one app, tasks in another, docs in a third, and the actual knowledge — the *why* behind decisions — scattered across all of them. Context dies in the gaps. EngineerOS is my attempt to fix that: notes, tasks, projects, and a knowledge graph in one connected system, with semantic search and an AI assistant that cites its sources. It is live, and 10+ people now use it daily — hard.',
                '',
                '## The Assumption That Was Wrong',
                '',
                'I started with a clean assumption: **the problem is storage.** If I put notes, tasks, and docs in one database with one schema, the fragmentation problem disappears. Build the unified data model, ship a nice UI on top, done.',
                '',
                'That assumption survived about two weeks of real use.',
                '',
                'Unifying storage does not unify *context*. You can have every note and task in one Postgres table and still not be able to answer "what did I decide about auth last month, and why?" The data was together, but it was not *connected* — and it was not *retrievable* in the way a human actually thinks. A shared schema is table stakes. The real problem was retrieval and relationships, not storage.',
                '',
                '## Where It Broke',
                '',
                'Three failures, in the order I hit them:',
                '',
                '**1. Keyword search was useless.** My first search was plain SQL `ILIKE`. Users search by *meaning*, not by exact words — they type "how did I handle rate limits" and the note says "throttling middleware." Keyword search returns nothing, and to the user the workspace looks empty even though the answer is right there. Fragmentation solved, discovery still broken.',
                '',
                '**2. The AI assistant hallucinated confidently.** Once I bolted an LLM on top, it happily invented answers that sounded right and were wrong. For a workspace people trust with real decisions, a confident wrong answer is worse than no answer — it erodes trust in the entire product on the first bad response.',
                '',
                '**3. Local-first felt great until search needed the server.** I wanted the snappy, offline-capable feel of a local-first notes app. But semantic search and the assistant need server-side compute and the vector index. Naively, every keystroke round-tripped to the server and the "snappy" app felt like molasses.',
                '',
                '## The Architecture',
                '',
                'The fixes shaped the system:',
                '',
                '- **Next.js 14 App Router with React Server Components** — the UI shell and reads render on the server where the data and the vector index live, so search does not pay a client round-trip per interaction. Interactive editing stays local-first for the snappy feel.',
                '- **Supabase (PostgreSQL + pgvector)** for auth, data, and semantic search — one store for both the relational workspace *and* the embeddings. Search runs an HNSW index over pgvector, so "how did I handle rate limits" retrieves the throttling note by meaning, at sub-200ms p95.',
                '- **Citation-backed assistant** — the assistant is retrieval-augmented: it answers *only* from retrieved workspace chunks and attaches the source note to every claim. No source, no claim. That single constraint is what turned it from a liar into something people trust.',
                '- **LangGraph multi-agent orchestration** for task workflows, with state checkpoints so a multi-step agent run is recoverable instead of restarting from zero on any failure.',
                '- **User-owned Supabase** — your data lives in *your* Supabase, not my black box. For a tool that holds your thinking, ownership is a feature, not a footnote.',
                '',
                'The knowledge graph ties it together: notes, tasks, and projects are nodes with real edges, so the assistant can traverse relationships ("this task came from that decision") instead of treating every record as an island.',
                '',
                '## The Hard Trade-offs',
                '',
                '- **Local-first UX vs. server-side intelligence.** The resolution was not picking one — it was drawing the line: edits are local and instant, intelligence (search, assistant) is server-side. Local-first UX with server-side intelligence is the right default for developer tools.',
                '- **Cost predictability with multi-step agents.** Multi-step LLM workflows can silently 10x your token bill. Checkpointing plus bounded agent steps keeps cost predictable instead of open-ended.',
                '- **Trust vs. helpfulness.** An assistant that says "I don’t have a note on that" beats one that guesses. Citations + provenance are what make an AI assistant safe to rely on for real engineering work.',
                '',
                '## The Result',
                '',
                'EngineerOS is live at [engineeros-delta.vercel.app](https://engineeros-delta.vercel.app/), source on [GitHub](https://github.com/KunjShah95/EngineerOS). It unifies notes, tasks, projects, and a knowledge graph, with semantic search at **< 200ms p95** and a citation-backed multi-agent assistant.',
                '',
                'The number I care about most is not latency — it is that **10+ people now use it daily and lean on it hard.** That is the real benchmark. A workspace only proves itself when people trust it with the thinking they cannot afford to lose. Watching real users search by meaning and get the right note back, with a citation, is the whole point.',
                '',
                'The lesson I keep relearning: **unifying storage is easy; unifying *context* is the actual product.** Meaning-based retrieval and provenance — not a shared schema — are what make a connected workspace feel connected.'
            ].join('\n'),
            date: 'AUG 2026',
            readTime: 9,
            tags: ['CASE STUDY', 'AGENTS', 'RAG', 'FULLSTACK', 'AI'],
            featured: true,
        },
];
