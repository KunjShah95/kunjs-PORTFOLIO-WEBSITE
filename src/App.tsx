      ...
      <section className="relative py-24 md:py-32 overflow-hidden bg-sunken/10 border-t border-rule/12">
        <div className="relative max-w-manifest mx-auto px-6">
          <motion.div
            ...
            className="mb-14"
          >
            <div className="kicker">How I build</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              <div className="flex items-center gap-2 mb-5">
                ...
                <span className="font-mono text-[11px] uppercase tracking-[0.14em] text-ink-tertiary">RAG Pipeline</span>
              </div>
              <div className="space-y-2 font-mono text-[13px] leading-relaxed">
                {[
                  'Documents → Chunking → Embedding',
                  'Vector store (ChromaDB) → Similarity search',
                  'Retrieved context + Query → LLM generation',
            ...
            >
              <div className="flex items-center gap-2 mb-5">
                ...
                <span className="font-mono text-[11px] uppercase tracking-[0.14em] text-ink-tertiary">Multi-Agent Architecture</span>
              </div>
              <div className="space-y-2 font-mono text-[13px] leading-relaxed">
                {[
                  ['M', 'Manager agent: decomposes goals → task graph'],
                  ['W', 'Worker agents: parallel execution with tool access'],
                  ['G', 'Guardrails: eval → HITL gates → fallback'],
                  ['✓', 'Checkpointing + state persistence (Postgres JSONB)'],
                ].map(([tag, step]) => (
                  <div key={tag} className="flex items-start gap-3">
                    <span className="text-accent/50 shrink-0 w-4 text-center">{tag}</span>
                    <span className="text-ink-secondary">{step}</span>
                  </div>
                ))}
              </div>
            ...
            >
              <div className="flex items-center gap-2 mb-5">
                ...
                <span className="font-mono text-[11px] uppercase tracking-[0.14em] text-ink-tertiary">LLM Internals: BPE Tokenizer</span>
              </div>
              <div className="space-y-2 font-mono text-[13px] leading-relaxed">
                {[
                  'Pre-tokenize: regex split (GPT-2/4 pattern)',
                  'BPE merge: learn ~50K merge rules from corpus',
                  'Encode: apply merges → token IDs',
                  'Decode: token IDs → byte sequences → text',
                ].map((step, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <span className="text-accent/50 shrink-0 tabular-nums">{i + 1}.</span>
                    <span className="text-ink-secondary">{step}</span>
