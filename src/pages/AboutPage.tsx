  ...
  return (
    <>
      ...
      <article className="relative max-w-manifest mx-auto px-6 py-24 overflow-hidden bg-sunken/5">
        {/* Dynamic ambient beams */}
        <BackgroundBeams count={3} className="opacity-30" />

        <TracingBeam className="max-w-prose mx-auto relative z-10">
          {/* Bio Section */}
          ...
          <section id="principles" className="pl-6 md:pl-10">
            ...
            <ol className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
                ...
                <li key={p.num} className="h-full">
                  <TiltCard scale={1.01} maxRotation={4} className="h-full">
                    <LiquidGlass intensity="subtle" className="p-6 rounded-2xl border border-rule/12 h-full flex flex-col">
                      <span className="font-mono text-xs text-accent font-bold tracking-wider">{p.num}</span>
                      <h3 className="display text-lg font-bold mt-2 text-ink-primary">{p.title}</h3>
          ...
          <section id="now" className="pl-6 md:pl-10">
            ...
            <LiquidGlass intensity="medium" className="p-6 md:p-8 rounded-2xl border border-rule/12">
              ...
              {SKILL_GROUPS.map((group, i) => {
                ...
                return (
                  <div key={group.category} className="h-full">
                    <TiltCard scale={1.015} maxRotation={5} className="h-full">
                      <LiquidGlass
                        intensity="subtle"
                        tint="rgba(124, 118, 255, 0.06)"
                        className="p-6 md:p-8 rounded-2xl border border-rule/12 hover:border-accent/30 transition-all flex flex-col justify-between h-full group"
                      >
                        <div>
                          <div className="flex items-center justify-between mb-5">
                            ...
                            {Icon && (
                              <Icon
                                className="w-5 h-5 text-ink-tertiary group-hover:text-accent transition-colors duration-300"
                                strokeWidth={1.5}
                                aria-hidden
                              />
                            )}
                          ...
                          <h3 className="display text-xl font-bold tracking-tight mb-2">
                            {group.category}
                          </h3>
                        ...
                        <div className="mt-auto pt-5 border-t border-rule/8">
                          ...
                          <ul className="flex flex-wrap gap-1.5">
                            {group.skills.map((skill) => (
                              <li
                                key={skill}
                                className="font-mono text-[11px] text-ink-primary px-2.5 py-1 border border-rule/8 rounded-full bg-paper/60 hover:bg-accent/10 hover:border-accent/30 transition-colors"
                              >
                                {skill}
                              </li>
                            ))}
                ...
                return (
                  <div key={l}>
                    <TiltCard scale={1.02} maxRotation={5}>
                      <LiquidGlass intensity="subtle" className="p-4 md:p-5 rounded-2xl border border-rule/12 text-center">
                        <div className="display text-2xl md:text-3xl text-accent font-extrabold tabular-nums">
                ...
                <div key={i}>
                  <TiltCard scale={1.01} maxRotation={3}>
                    <SpotlightCard className="rounded-2xl">
                      <LiquidGlass
                        intensity="subtle"
                        tint="rgba(124, 118, 255, 0.05)"
                        className="p-6 md:p-8 border border-rule/12 flex flex-col md:flex-row md:items-start gap-4 md:gap-8 justify-between"
                      >
                        <div className="md:w-1/4 shrink-0">
                          <span className="font-mono text-xs text-accent font-bold bg-accent/8 border border-accent/20 px-2.5 py-1 rounded-md">
                            {exp.period}
                          </span>
                        </div>
                        <div className="flex-1">
                          <h3 className="display text-xl font-bold text-ink-primary">{exp.role.replace(/_/g, ' ')}</h3>
                          <div className="font-mono text-sm text-accent font-bold mt-1.5">{exp.company.replace(/_/g, ' ')}</div>
                          <p className="mt-4 text-sm md:text-base text-ink-secondary leading-relaxed">{exp.description}</p>
                          {exp.skills && (
                            <div className="mt-4 pt-4 border-t border-rule/8 flex flex-wrap gap-1.5">
                              {exp.skills.map(skill => (
                                <span key={skill} className="px-2 py-0.5 rounded bg-paper/60 border border-rule/8 text-[10px] font-mono text-ink-secondary">
                                  {skill.replace(/_/g, ' ')}
                                </span>
                              ))}
                            </div>
